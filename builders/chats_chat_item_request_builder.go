...
// ChatsChatItemRequestBuilder provides operations to manage the collection of chat entities.
...
// ChatsChatItemRequestBuilderGetQueryParameters retrieve a single chat (without its messages). This method supports federation. To access a chat, at least one chat member must belong to the tenant the request initiated from.
