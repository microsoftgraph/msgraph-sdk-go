...
func (m *DeltaRequestBuilder) Get(options *DeltaRequestBuilderGetOptions)([]ContactFolder, error) {
    requestInfo, err := m.CreateGetRequestInformation(options);
    if err != nil {
        return nil, err
    }
    res, err := m.requestAdapter.SendCollectionAsync(*requestInfo, func () i04eb5309aeaafadd28374d79c8471df9b267510b4dc2e3144c378c50f6fd7b55.Parsable { return NewContactFolder() }, nil, nil)
    if err != nil {
        return nil, err
    }
    val := make([]ContactFolder, len(res))
    for i, v := range res {
        val[i] = *(v.(*ContactFolder))
    }
    return val, nil
}
