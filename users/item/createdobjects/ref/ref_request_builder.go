...
// refRequestBuilder builds and executes requests for operations under \users\{user-id}\createdObjects\$ref
...
// refRequestBuilderGetQueryParameters directory objects that were created by the user. Read-only. Nullable.
...
func NewRefRequestBuilderInternal(pathParameters map[string]string, requestAdapter ida96af0f171bb75f894a4013a6b3146a4397c58f11adb81a2b7cbea9314783a9.RequestAdapter)(*RefRequestBuilder) {
    ...
    m.urlTemplate = "{+baseurl}/users/{user_id}/createdObjects/$ref{?top,skip,search,filter,count,orderby}";
...
// CreatePostRequestInformation directory objects that were created by the user. Read-only. Nullable.
...
// Post directory objects that were created by the user. Read-only. Nullable.
