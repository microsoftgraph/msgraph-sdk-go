...
// refRequestBuilder builds and executes requests for operations under \users\{user-id}\ownedObjects\$ref
...
// refRequestBuilderGetQueryParameters directory objects that are owned by the user. Read-only. Nullable. Supports $expand.
...
func NewRefRequestBuilderInternal(pathParameters map[string]string, requestAdapter ida96af0f171bb75f894a4013a6b3146a4397c58f11adb81a2b7cbea9314783a9.RequestAdapter)(*RefRequestBuilder) {
    ...
    m.urlTemplate = "{+baseurl}/users/{user_id}/ownedObjects/$ref{?top,skip,search,filter,count,orderby}";
...
// CreatePostRequestInformation directory objects that are owned by the user. Read-only. Nullable. Supports $expand.
...
// Post directory objects that are owned by the user. Read-only. Nullable. Supports $expand.
