...
// RefRequestBuilder builds and executes requests for operations under \users\{user-id}\ownedDevices\$ref
...
// RefRequestBuilderGetQueryParameters devices that are owned by the user. Read-only. Nullable. Supports $expand.
...
func NewRefRequestBuilderInternal(pathParameters map[string]string, requestAdapter ida96af0f171bb75f894a4013a6b3146a4397c58f11adb81a2b7cbea9314783a9.RequestAdapter)(*RefRequestBuilder) {
    ...
    m.urlTemplate = "{+baseurl}/users/{user_id}/ownedDevices/$ref{?top,skip,search,filter,count,orderby}";
...
// CreatePostRequestInformation devices that are owned by the user. Read-only. Nullable. Supports $expand.
...
// Post devices that are owned by the user. Read-only. Nullable. Supports $expand.
